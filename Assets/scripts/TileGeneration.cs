...
public class TileGeneration : MonoBehaviour
{
  ...
  public TileData GenerateTile(float centerVertexZ, float maxDistanceZ)
  {
    ...
    // calculate the offsets based on the tile position
    float[,] heightMap = this.noiseMapGeneration.GeneratePerlinNoiseMap(tileDepth, tileWidth, this.mapScale, offsetX, offsetZ, heightWaves);
    ...
    TileData tileData = new TileData(heightMap, heatMap, moistureMap, chosenHeightTerrainTypes, chosenHeatTerrainTypes, chosenMoistureTerrainTypes, chosenBiomes, this.meshFilter.mesh, biomeTexture);
    return tileData;
...
public class TileData
{
  ...
  public TileData(float[,] heightMap, float[,] heatMap, float[,] moistureMap,
    ...
    Biome[,] chosenBiomes, Mesh mesh, Texture2D texture2D)
  {
